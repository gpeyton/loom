...
echo "  ✅ .claude/ directory with slash commands (Claude Code)"
echo "  ✅ .codex/ directory with config, prompt shims, and custom agents (Codex)"
echo "  ✅ .agents/skills/ Codex skills — \$loom and \$loom-sweep entry points"
...
echo ""
echo "  Claude Code and the OpenAI Codex CLI are co-equal worker runtimes."
echo "  Both get unrestricted permissions by default for Loom-spawned"
echo "  automation; set LOOM_CODEX_SAFE=1 to opt Codex back into a sandboxed"
echo "  posture. See defaults/.codex/GUARDRAIL-PARITY.md for the trust"
echo "  boundary this default assumes."
echo ""
...
case "$MERGE_STATUS" in
  merged)
    ...
    echo "  Loom is ready to use! Choose your runtime and workflow:"
    echo ""
    echo "  Manual Mode, Claude Code (recommended to start):"
    echo "    cd $TARGET_PATH && claude"
    echo "    Then use /builder, /judge, or other role commands"
    echo ""
    echo "  Manual Mode, Codex:"
  ...
  auto)
    ...
    echo ""
    echo "  Once merged, choose your runtime and workflow:"
    echo ""
    echo "  Manual Mode, Claude Code (recommended to start):"
    echo "    cd $TARGET_PATH && claude"
    echo "    Then use /builder, /judge, or other role commands"
  ...
  *)
    ...
    echo "  2. Choose your runtime and workflow:"
    echo "     Manual Mode, Claude Code (recommended to start):"
    echo "       cd $TARGET_PATH && claude"
    echo "       Then use /builder, /judge, or other role commands"
    echo "     Manual Mode, Codex:"
