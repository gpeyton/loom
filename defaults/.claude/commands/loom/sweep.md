...
**Backend choice is explicit, not silent (issue #54).** Once `RUNTIME` is resolved, and — for `runtime == codex` — once the Codex sub-strategy is chosen (sequential-in-session vs. `spawn-codex-wave.sh` process-level fan-out vs. daemon dispatch), **announce the chosen backend in the first status line of the run** (e.g., `Backend: codex / process-level fan-out (spawn-codex-wave.sh), wave size 3` or `Backend: claude / subagent dispatch`) and **repeat it in the final summary output**. This is not cosmetic: umbrella issue #51's incident showed a Codex session silently switch orchestration mechanisms mid-run with no announcement at all, which is exactly the ambiguity that let native-agent usage go unnoticed until the postmortem. A visible, one-line backend declaration at start and in the summary is cheap and closes that gap for every runtime and every mode (A/B/C).
...
Codex has no Claude-Code-style Task-tool subagents — there is nothing to dispatch a `loom-builder` / `loom-judge` / `loom-doctor` *into* the way this session dispatches Claude subagents. Parallelism, when it exists at all, is process-level (`codex exec` children or daemon-dispatched sweeps). For a single interactive Codex sweep the strategy is therefore:
...
**Correcting a stale capability claim.** Earlier revisions of this section said "Codex has no subagents" as if that were a technical impossibility, and reasoned from that premise to "therefore there is nothing to route parallel Loom work into except process-level fan-out." **That premise is now false.** Current Codex clients expose native, in-session collaboration primitives — `spawn_agent`, `wait_agent`, `send_message`, `followup_task`, `interrupt_agent` — that let a root Codex session spawn and supervise other in-session agents, no `codex exec` child process required. A real incident (documented in umbrella issue #51, reproduction B) showed a root Codex session asked for "parallel Loom agents" choose those native primitives instead of `spawn-codex-wave.sh`, mixing Loom's lifecycle/checkpoint/ownership semantics with generic native-agent supervision — the same unsafe polling/interruption/takeover behavior the "Codex Child Supervision Contract" section below exists to prevent, but reached through a path that contract's `spawn-codex-wave.sh`-specific mechanics don't touch at all.

**The accurate statement, going forward:** native Codex agent primitives exist, but they are **not yet a supported Loom orchestration backend**. This is an explicit policy choice, not a technical gap — Loom has deliberately not built role-specific native-agent prompts, checkpoint mapping, ownership contracts, or resume semantics for that backend (that would be "Path B" of #54, tracked as a separate future feature, not implemented here). Until a future issue formally adopts native agents as a second supported backend, with its own ownership/checkpoint/outcome contract, the rule is:

- **Process-level fan-out (`spawn-codex-wave.sh`) is the ONLY supported backend for parallel Loom role/lifecycle dispatch under Codex.** A single interactive Codex sweep runs the sequential-in-session strategy above; multi-issue parallelism runs through `spawn-codex-wave.sh` under `LOOM_CODEX_MULTI_WAVE=1` (see "Multi-wave process-level Codex orchestration" below) — nothing else.
- **Any request for "parallel Loom agents" — however phrased — routes to `spawn-codex-wave.sh`, never to native `spawn_agent`-style primitives.** This applies regardless of whether the native primitives are technically available in the running Codex client. Availability is not the same as being a supported Loom backend. For example, a user prompt phrased as *"execute with up to 3 parallel Loom agents"* means: invoke `LOOM_CODEX_MULTI_WAVE=1 ./.loom/scripts/spawn-codex-wave.sh <issue> <issue> <issue>` (one process-level child per issue, wave size clamped to the candidate count), not `spawn_agent` calls onto the `.codex/agents/loom-*.toml` custom agents.
- **A single Codex run must not silently mix orchestration mechanisms.** Within one sweep invocation, pick exactly one of: (a) sequential-in-session (this Codex strategy, no dispatch at all), (b) process-level fan-out (`spawn-codex-wave.sh`), or (c) daemon dispatch (`mcp__loom__dispatch_sweep`, Stage -1's `use_daemon` path). Do **not** start a `spawn-codex-wave.sh` wave and *also* spawn native in-session agents for the same or overlapping issues, and do not fall back to inline role work (the root session doing Builder work itself) while a process-level or native-agent child for that issue is still alive — that inline-takeover failure mode is already forbidden by the "Codex Child Supervision Contract" below regardless of which mechanism spawned the child.
- **This prohibition is binding on every Codex orchestration surface**, not just this skill: the daemon child-prompt encoding (`encode_child_prompt` in `sweep_registry.rs`), the installed `.agents/skills/loom-sweep/SKILL.md` router, the `.codex/agents/loom-orchestrator.toml` custom agent, and the deprecated `.codex/prompts/loom-sweep.md` shim all carry the same statement — see each file for its restatement of this policy.
- **Path B (native agents as a fully supported second backend) is explicitly out of scope here.** If Loom ever adopts native Codex agents as a first-class backend, that requires its own follow-up issue defining role-specific prompts, one-level-deep dispatch, full-wave join semantics, checkpoints, worktree ownership, structured outcomes, and cancellation provenance — full parity with this section's process-level path, not a partial one. Until that lands, native primitives stay unused for Loom lifecycle work even though the running Codex client may expose them.
...
Issue #19 landed **runtime detection + a single-role / single-issue sequential Codex sweep** (Curator → Builder → Judge → Doctor → Merge, one session, no Task subagents) plus the runtime-aware daemon child-prompt encoding. That left `--builders-per-wave N > 1` degrading to sequential per-issue processing under Codex — there was no subagent to parallelize into. Issue #24 (unblocked by guardrail parity, issue #20) resolves that gap with **true process-level fan-out**: multiple `codex exec` children, run as independent OS processes, are the process-level analogue of Claude's `--builders-per-wave` parallel waves.

- **Coordination substrate: direct `codex exec` child OS processes, not `mcp__loom__dispatch_sweep`.** The daemon's dispatch surface is issue-keyed only in v0.10.0 (`dispatch_sweep --kind '{"Issue":N}'`) with no wave/batch primitive, so routing wave-level fan-out through it would require daemon-side changes. Direct process spawning needs no daemon running, mirrors `spawn-codex.sh`'s existing single-child model (this is "N `spawn-codex.sh` invocations, run concurrently, with a join"), and is the lower-risk substrate. A future issue may revisit daemon-routed Codex waves if the dispatch surface grows a batch primitive; today it does not.
- **The mechanism**: `defaults/scripts/spawn-codex-wave.sh <issue-number> [<issue-number> ...]` takes a wave of issue numbers, spawns one `codex exec` child per issue (each running the existing single-role sequential Codex sweep lifecycle via `spawn-codex.sh` + the `.codex/prompts/loom-sweep.md` shim), and blocks until **every** child in the wave has exited before returning — collecting per-issue exit codes and per-issue logs (`.loom/logs/spawn-codex-wave-issue-<N>.log` by default).
- **Wave settling discipline is preserved.** The #3289 stream-pump stall hazard is Claude-specific and does not apply to independent OS processes, but the *sequencing policy* #3289 also enforces — each wave fully settles before the next starts — carries over by construction: `spawn-codex-wave.sh` IS the settling boundary for one wave. A caller (this skill's Codex strategy, or an operator script) that invokes it once per wave and waits for it to return gets the same "wave N+1 does not start until wave N has fully settled" guarantee the Claude wave loop provides.
- **Opt-in gate: `LOOM_CODEX_MULTI_WAVE=1`, orthogonal to `LOOM_CODEX_UNSAFE`.** `LOOM_CODEX_UNSAFE` (issue #20 / `spawn-codex.sh`) gates *permissions posture* (sandbox vs. bypass-everything); `LOOM_CODEX_MULTI_WAVE` gates *concurrency* (how many children run at once) — a separate concern. Without `LOOM_CODEX_MULTI_WAVE=1`, a wave with more than one issue **degrades to sequential processing**, one issue at a time — today's #19 behavior, unaffected by this script's existence. With the opt-in set, all children in a multi-issue wave fan out concurrently. Consistent with #19/#20's discipline: autonomous multi-wave Codex sweeps with write access remain opt-in and loudly documented — this script does not touch `spawn-codex.sh`'s permission posture at all, and a single-issue wave is always effectively sequential regardless of the gate.
...
For native Codex collaboration agents (`spawn_agent` / `wait_agent` / `interrupt_agent`-style primitives): issue #54 resolved the forward reference this paragraph used to carry. **Decision (Path A, #54): native Codex agent primitives are explicitly NOT a supported Loom orchestration backend** — see "Codex backend policy" above for the full rationale and the routing rule that sends every "parallel Loom agents" request to `spawn-codex-wave.sh` instead. This paragraph now exists purely as a forward-compat safety net: **if** a future issue formally adopts native agents as a second backend (Path B of #54, out of scope today), `interrupt_agent` must map to a cancellation outcome (`cancelled_by_operator` or `cancelled_by_parent` as appropriate), never to `failed`, and replacement/takeover of a target that is still alive is prohibited under the same rule as point 4 above — that future backend inherits this contract's outcome taxonomy rather than reinventing one. Until that future issue lands, this paragraph is not authorizing native-agent usage; it is only pre-registering the taxonomy mapping for if/when it's authorized.
...
### Recovery after cancellation (issue #53 — atomic reclaim, shipped)

When a cancellation genuinely was warranted (one of the three reasons in the patience contract above), the correct next step is to **rerun the sweep for the affected issue** so it can resume from its last completed phase (per the existing checkpoint mechanism, #3373) and reclaim its existing worktree. Issue #53 ("atomic reclaim of interrupted Builder claims") closed the gap this section used to describe as a forward reference: a `loom:building` label alone cannot distinguish a live owner from a dead one, so a naive rerun used to silently no-op (exit 0, no Builder work) instead of reclaiming. The fix is a small, atomically-guarded ownership lease, `defaults/scripts/sweep-claim.sh`, consumed directly by `spawn-codex-wave.sh`'s own preflight:

- **Lease file**: `.loom/sweep-claims/issue-<N>.json` — `{issue, run_id, runtime, pid, status, claimed_at, last_seen_at}`. `status` is one of `active` (a real owner holds it), `resumable` (safe to reclaim — the prior owner released it or was released on its behalf after a cancellation/failure), or `released` (terminal, e.g. after `merge-done`).
- **Liveness definition**: a claim is LIVE iff `status == "active"` AND `kill -0 <pid>` succeeds. Every other combination — no claim file at all, `resumable`/`released`, or `active` with a dead pid — is ORPHANED and safe to reclaim. No time-based guessing is involved in this determination (contrast with `stale-building-check.sh`'s fallback heuristic for issues predating this mechanism — see below).
- **Atomicity**: `sweep-claim.sh acquire` runs its read-check-write under a short-lived mkdir lock at `.loom/locks/claim-issue-<N>/` (mkdir is the only POSIX-atomic primitive on stock macOS — same precedent as `loom_tools.tokens.bad_tokens` and `worktree.sh`'s `acquire_worktree_lock`; `flock` is not used). Two concurrent `acquire` calls for the same issue race on the mkdir syscall itself: exactly one wins; the loser re-reads the now-updated lease and correctly refuses rather than overwriting. This is the guarantee "two concurrent retries must not both become Builder" requires.
- **`spawn-codex-wave.sh`'s preflight** (the process-level Codex path this contract already governs): before spawning a child for issue `N`, it calls `sweep-claim.sh acquire N --pid $$ --runtime codex`, using its OWN pid as the lease's liveness anchor (valid because the wave script always forks the child and then synchronously `wait`s on it — its own lifetime is a superset of the child's). If acquire is refused (a live claim exists), **no child is spawned** and the per-issue outcome is `skipped` — a bucket distinct from `completed`/`failed`/`cancelled_by_*`/`unknown` in both the structured status file and the outcome-breakdown summary line, so a genuine no-op is never confused with a successful run. If acquire succeeds (no claim, or an orphaned one), a child IS spawned and — because `worktree.sh` is idempotent — it re-enters the same worktree and reuses whatever partial diff a prior, now-dead child left behind.
- **Release on terminal outcome**: the wave script itself (not the child, which may be dead or unable to run cleanup) releases the lease once the child's outcome is known: `completed` → `released`; `failed` or any `cancelled_by_*` → `resumable`. This directly implements "the owning runner releases the claim to a resumable state" — mechanically, from the process that's still alive to do it.
- **`stale-building-check.sh`** (the operator-facing recovery report, `.loom/docs/troubleshooting.md` / CLAUDE.md's "Quick fixes") consults `sweep-claim.sh is-live` as its primary signal and falls back to the pre-#53 time-based heuristic (`STALE_THRESHOLD_HOURS`) only for issues with no claim on record at all (legacy state, or a runtime that never registered one). `--recover` resets the label and releases the claim to `resumable`; issues with a genuinely live claim, or with an open linked PR in flight, are never touched.
- A `cancelled_by_operator` / `cancelled_by_parent` / `cancelled_by_deadline` outcome on a Builder child means the issue's `loom:building` claim and worktree are in a **recoverable, not corrupt** state — the lease's `resumable` status is exactly that signal, consumed by the next `acquire` call without any operator intervention required.
...
- Expected exit states:
  ...
  - **Request changes** → PR labeled `loom:changes-requested` by Judge. Continue to **C1b (Doctor → Judge)** for this PR (single inline cycle, matching the issue-side cap).
    If a closing-issue checkpoint is in scope, write `judge-rejected` before
    entering C1b so an interrupted sweep resumes at Doctor rather than
    repeating the completed Judge pass:
    ```bash
    ./.loom/scripts/sweep-checkpoint.sh write N judge-rejected --task-id "sweep-$$" --pr-number P
    ```
...
- If `CHECKPOINT_PHASE == "judge-rejected"`, the initial Judge already completed and requested changes. Do **not** re-run Judge; route directly to the single Doctor→Judge cycle in step 6. The checkpoint's `pr_number` is the routing key and the PR should still carry `loom:changes-requested`; if it does not, log a forge/checkpoint divergence warning and re-run Judge.
...
- Expected exit states per PR:
  - **Approve** → PR labeled `loom:pr`. Write the `judge-done` checkpoint for this issue (carrying the PR number), then continue to Merge (step 7) for this PR, then advance to the next PR in the wave.
    ...
    ```
  - **Request changes** → PR labeled `loom:changes-requested`. Write `judge-rejected` before continuing to Doctor (step 6) **inline for this PR**, then re-judge, then merge or block. Do **not** write a `judge-done` checkpoint here — the PR is not yet approved.
    ```bash
    ./.loom/scripts/sweep-checkpoint.sh write N judge-rejected --task-id "sweep-$$" --pr-number <PR>
    ```
...
If Judge requests changes on PR `#X` mid-wave, or `CHECKPOINT_PHASE == "judge-rejected"` resumes a completed rejection, run a **single inline Doctor→Judge cycle** for `#X` before moving to the next PR's Judge. A resumed `judge-rejected` checkpoint must not dispatch another initial Judge.
...
When the entire list has been processed, print a summary table that includes wave membership for each issue. **The first line of the summary restates the backend used for the run** (issue #54 — see "Backend choice is explicit, not silent" in Runtime detection above), so a reader never has to infer it from the transcript:

```
Backend: claude / subagent dispatch
