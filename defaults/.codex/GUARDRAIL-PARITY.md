...
| Foreground-turn liveness | While Loom delivery is outstanding, the Codex supervisor keeps its turn open on the blocking join or `spawn-codex-wave.sh --monitor`. The monitor reads local structured state with bounded backoff, makes zero recurring GitHub calls, and an observer stop never rewrites child state. | **UI control has no equivalent.** Loom can keep a turn pending so Codex shows its normal Working indicator, but cannot pin or recreate that product UI after the turn ends. The terminal-condition rule is documented and `--monitor` is mechanical; the indicator itself remains Codex-owned. |
...
| Native agents are not a supported backend | Current Codex clients expose native, in-session collaboration primitives (`spawn_agent`, `wait_agent`, `send_message`, `followup_task`, `interrupt_agent`). Loom's policy (#54, Path A — the safety baseline shipped now): these primitives exist but are **not yet a supported Loom orchestration backend** for role/lifecycle dispatch. Any request for "parallel Loom agents" routes to `spawn-codex-wave.sh`, never to `spawn_agent`. A single Codex run must not mix sequential-in-session, process-level fan-out, daemon dispatch, and native-agent spawning. | **No native enforcement.** Same class of gap as the rows above — a documented convention, not an OS boundary. Codex has no policy hook that can mechanically block a session from calling `spawn_agent`; the prohibition is enforced by `.claude/commands/loom/sweep.md`'s "Codex backend policy" section, this row, `.agents/skills/loom-sweep/SKILL.md`, and the daemon child-prompt text, not by a sandbox rule. A fully-supported native-agent backend (Path B of #54) is explicit future work, not implemented today. |
| Native-agent interrupt mapping (forward-compat only) | **If** a future issue formally adopts native Codex collaboration agents as a second supported backend (Path B of #54): `interrupt_agent` must map to a cancellation outcome, never `failed`, and must not replace/take over a still-alive target — inheriting the taxonomy above rather than reinventing one. | Forward-looking only, and **not currently authorized** — see the row above. This row is pre-registered so a future Path B lands with the taxonomy already decided, not so it's safe to use native agents today. |

**Bottom line**: unlike the sandbox/approval rows above (which are enforced by Codex's own process boundary), the supervision guardrails in this section are enforced by (a) `spawn-codex-wave.sh`'s implementation for the mechanical parts it can own (no inactivity timeout, signal-provenance tagging, a `--status` read path that is read-only by construction, and a local-only foreground `--monitor`), and (b) documentation + operator/agent discipline for the parts no current Codex primitive can mechanically block (worktree non-interference across sessions, refusing to poll aggressively, and not ending the foreground turn while work remains). Treat this the same way you treat the rest of Codex's trust boundary: know where the enforced edge is, and don't rely on an unenforced convention as if it were a sandbox.
