...
#
# Tiers 2/4 only resolve inside an actual loom checkout; a CONSUMER repo's
# installed .loom/scripts/spawn-claude.sh has no loom-tools/ sibling, so it
# relies on tier 1 or the recorded source path at tier 3. Issue #3949:
# `loom-daemon`'s `spawn_child` (sweep_registry.rs::resolve_package_path_env)
# now sets LOOM_PACKAGE_PATH automatically on every daemon-dispatched child —
# derived from the loom checkout the running daemon binary was built from — so
# it no longer needs to be exported manually before starting the daemon.
